...
tf.app.flags.DEFINE_string("save_dir","results","Saving directory")
tf.app.flags.DEFINE_string("load_dir","load","loading directory")
...
def save_3d_file(dataSet):
  data_dir = FLAGS.save_dir
  np.save('results.npy',dataSet)
...
def predict():
  ...
  with tf.Session(config=tf.ConfigProto(
    ...
    path = FLAGS.load_dir
    test_in_data = np.load(path)
    test_in_frame = test_in_data.shape[0]
    test_in_data_mean, test_in_data_std = data_pre.normalizeStats(test_in_data)
    test_in_data_norm = data_pre.normalizeData(test_in_data, test_in_data_mean, test_in_data_std, test_in_frame)
